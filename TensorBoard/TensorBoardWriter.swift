        ...
        for stat in stats {
            ...
            if let stored = summaryWriters[writerKey] {
                ...
                writer = SummaryWriter(
                    logDirectory: URL(fileURLWithPath: logRootDirectory, isDirectory: true)
                        .appendingPathComponent(logDirTimestampPrefix).appendingPathComponent(
                            writerKey
                        ).path)
            ...
            globalQueue.async {
                if batchIndex + 1 == batchCount {
                    writer.addScalar(
                        tag: "epoch_" + stat.name, value: stat.value, step: Int64(epochIndex))
                }
                if let globalTrainBatchIndex = globalTrainBatchIndex {
                    writer.addScalar(
                        tag: "batch_" + stat.name, value: stat.value,
                        step: Int64(globalTrainBatchIndex))
                }
            }
        }
    }
