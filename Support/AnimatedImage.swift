...
struct AnimatedImage {
    ...
    public init(_ frames: [Tensor<Float>]) {
        ...
        guard frames[0].rank == 3 else {
            fatalError(
                "Rank-3 tensors are needed, and rank \(frames[0].rank) frames were provided.")
        }
...
extension Array where Element == Tensor<Float> {
    ...
    ///   - delay: The delay (in hundredths of a second) to insert between each frame.
    ///   - loop: Whether the animation should loop indefinitely.
    public func saveAnimatedImage(directory: String, name: String, delay: Int, loop: Bool = true)
        throws
    {
