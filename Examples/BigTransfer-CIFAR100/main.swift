...
func getSchedule(datasetSize: Int) -> [Int] {
...
let classCount = 100
var bitModel = BigTransfer(
    classCount: classCount, depth: getModelUnits(modelName: modelName), modelName: modelName)
...
for (epoch, batches) in dataset.training.prefix(epochCount).enumerated() {
    ...
    for batch in batches {
        if let newLearningRate = getLearningRate(
            step: currStep, datasetSize: cifar100TrainingSize, baseLearningRate: 0.003)
        {
        ...
        // var mixedUp = flipped
        var newLabels = Tensor<Float>(
            Tensor<Int32>(oneHotAtIndices: eagerLabels, depth: classCount))
        //if mixupAlpha > 0.0 {
