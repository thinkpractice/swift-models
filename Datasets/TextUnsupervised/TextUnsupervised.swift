...
public struct TextUnsupervised<Entropy: RandomNumberGenerator> {
        ...
        do {
            ...
            training = TrainingEpochs(
                ...
                batches.lazy.map {
                    LabeledData(
                        data: Tensor<Int32>(
                            $0.map(\.first).map { Tensor<Int32>($0.scalars, on: device) }),
                        label: Tensor<Int32>(
                            $0.map(\.second).map { Tensor<Int32>($0.scalars, on: device) })
                    )
                }
            }
    ...
    private static func downloadIfNotPresent(
        ...
        let _ = DatasetUtilities.downloadResource(
            filename: (downloadEncodedFile
                ? variantDetails.encodedFileName! : variantDetails.filename),
    ...
    private static func loadDirectory(
        ...
        if let bpe = bpe {
            ...
            let pathPrefix = directory.appendingPathComponent(
                "\(variantDetails.encodedFileName!)/\(name)"
            ).path
            encodedDocs = (0..<documentCount).map {
                URL(fileURLWithPath: "\(pathPrefix)/doc_\($0).txt")
            }
