...
public struct CIFAR10<Entropy: RandomNumberGenerator> {
    ...
    public typealias Validation = LazyMapSequence<
        Slices<[(data: [UInt8], label: Int32)]>, LabeledImage
    >
    /// The training epochs.
    ...
    public init(
        ...
        if normalizing {
            ...
            standardDeviation = Tensor<Float>(
                [0.2470322324, 0.2434851280, 0.2615878417], on: device)
        }
        ...
        training = TrainingEpochs(samples: trainingSamples, batchSize: batchSize, entropy: entropy)
            .lazy.map { (batches: Batches) -> LazyMapSequence<Batches, LabeledImage> in
                return batches.lazy.map {
                    makeBatch(
                        samples: $0, mean: mean, standardDeviation: standardDeviation,
                        device: device)
                }
            }
