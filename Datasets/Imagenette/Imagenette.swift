...
public struct Imagenette<Entropy: RandomNumberGenerator> {
    ...
    public init(batchSize: Int, entropy: Entropy, device: Device) {
        self.init(
            ...
            inputSize: ImagenetteSize.resized320,
            outputSize: 224)
    }
    ...
    public init(
        ...
        do {
            let trainingSamples = try loadImagenetteTrainingDirectory(
                inputSize: inputSize, localStorageDirectory: localStorageDirectory,
                base: "imagenette")
...
extension Imagenette: ImageClassificationData where Entropy == SystemRandomNumberGenerator {
    ...
    public init(
        batchSize: Int, inputSize: ImagenetteSize, outputSize: Int,
...
{
    return try loadImagenetteDirectory(
        named: "val", in: localStorageDirectory, inputSize: inputSize, base: base,
        labelDict: labelDict)
}
